...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, AvailabilityCalendarComponent],
...
export class InterventionDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      // Charger la liste de main-d'œuvre uniquement pour les techniciens
  ...
  loadTaches(interventionId: number): void {
    ...
    service.subscribe({
      next: (data) => {
        this.taches = data || [];
        // Trier par ordre
        this.taches.sort((a, b) => (a.ordre || 0) - (b.ordre || 0));
      },
      error: (err) => {
  ...
  loadMainDOeuvre(): void {
    this.mainDOeuvreService.getAll({ disponibilite: 'DISPONIBLE' }).subscribe({
      next: (data) => {
  ...
  filtrerMainDOeuvreDisponible(): void {
    ...
    this.mainDOeuvreListeFiltree = this.mainDOeuvreListe.filter(md => {
      ...
      const matchDisponibilite = !this.filtreDisponibiliteMainDOeuvre ||
        (md.disponibilite || 'DISPONIBLE') === this.filtreDisponibiliteMainDOeuvre;
      ...
      // Exclure ceux déjà affectés
      const dejaAffecte = this.mainDOeuvreAffectee.some(ma => ma.id === md.id);
      
      return matchRecherche && matchCompetence && matchDisponibilite && matchHabilitation && !dejaAffecte;
    });
  }
    ...
    this.mainDOeuvreService.verifierAffectation(this.intervention.id, mainDOeuvreId).subscribe({
      next: (verification) => {
        ...
        if (!isVerificationValide(verification)) {
          ...
          verification.erreurs.forEach(erreur => {
            message += `  • ${erreur}\n`;
          });
          ...
          if (verification.avertissements && verification.avertissements.length > 0) {
            message += '\n⚠️ Avertissements :\n';
            verification.avertissements.forEach(avert => {
              message += `  • ${avert}\n`;
            });
          }
  ...
  getMainDOeuvreName(mainDOeuvreId: number | undefined): string {
    ...
    const md = this.mainDOeuvreAffectee.find(m => m.id === mainDOeuvreId);
